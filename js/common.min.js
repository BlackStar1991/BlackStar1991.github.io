window.onload = function () {
    ...
    var
        ...
        navigation = $(".bl_nav"),
        navigationItemLength = $(".bl_nav__item").length,
    ...
    likeLink($(".js-likeLink"));


    sliderEffectForButtons(btnSearch, searchFild, 500);
    sliderEffectForButtons($(".btn_moreSymbolInfo"), $(".game_symbol__wrapper"), 500);

    if (body.width() > 991) {

        if(navigationItemLength > 7){
            navigation.slick({
                dots: false,
                infinite: true,
      ...
      if($(".bl_slots__category").length !== 0){
          $('.bl_topGames').slick({
              dots: false,
              infinite: true,
    ...
    $(".btn_bonus__info").on("click", function () {
        fildDescBonuses.addClass(active);

        var index = $(this).closest(".bl_bonus").data("index");
            index = index - 1;


        descCasino.removeClass(active);
        descCasino.eq(index).addClass(active);
