...
function setHrefForSubmitButton(selector, appName, route) {
    var data = getQueryData(appName);
    var mirrorsUrl = data.mirror;
    var utmMetrics = getUtmMetrics();
    var getParams = data.params + utmMetrics;
    if (route !== undefined) {
        selector.attr('href', mirrorsUrl + route + getParams);
    } else {
        selector.attr('href', mirrorsUrl + '/' + getParams);
    }
}
...
var Hashids = function () {
    ...
    Hashids.prototype._encode = function (numbers) {
        var ret, lottery, i, len, number, buffer, last, sepsIndex, guardIndex, guard, halfLength, excess,
            alphabet = this.alphabet, numbersSize = numbers.length, numbersHashInt = 0;
        for (i = 0, len = numbers.length; i !== len; i++) {
    ...
    Hashids.prototype._decode = function (hash, alphabet) {
        var ret = [], i = 0, lottery, len, subHash, buffer, r = new RegExp("[" + this.guards + "]", "g"),
            hashBreakdown = hash.replace(r, " "), hashArray = hashBreakdown.split(" ");
